...
PVOID GetSockExtAPI(SOCKET sock, GUID guidFn)
{
	...
	(
		...
		&guid, sizeof(guid), &pfnEx, sizeof(pfnEx), &dwBytes, NULL, NULL
	);
...
void _tmain()
{
	...
	SOCKET hsoListen = GetListenSocket(9002, 1);
	if (hsoListen == INVALID_SOCKET)
	{
		WSACleanup();
		return;
	...
	(
		hsoListen, sock, szBuff, dwRecvLen, sizeof(SOCKADDR_IN) + 16, sizeof(SOCKADDR_IN) + 16,
		NULL, &ov
	);
	...
	pfnGetSockAddr
	(
		szBuff, dwRecvLen, sizeof(SOCKADDR_IN) + 16, sizeof(SOCKADDR_IN) + 16,
	...
	printf("    Client %s :%d -> Server %s: %d ", inet_ntoa(saRem.sin_addr), htons(saRem.sin_port), inet_ntoa(saLoc.sin_addr), htons(saLoc.sin_port));
	if (dwRecvLen > 0)
	{
