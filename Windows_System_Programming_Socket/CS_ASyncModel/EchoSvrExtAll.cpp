...
int IncreaseAcceptSockets(SOCKET hsoListen, int nIncCnt, SOCK_SET& pool)
{
	...
	for (; nPooledCnt < nIncCnt; nPooledCnt++)
	{
		...
		(
			hsoListen, sock, psi->_buff, 0, sizeof(SOCKADDR_IN) + 16, sizeof(SOCKADDR_IN) + 16,
		...
		if (dwWaitRet == 1)
		{
			...
			else if(msg.message == TM_SOCK_DISCONNECTED)
			{
				...
				else
				{
					...
					BOOL bIsOK = pfnAcceptEx
					(
						hsoListen, psi->_sock, psi->_buff, 0, sizeof(SOCKADDR_IN) + 16, sizeof(SOCKADDR_IN) + 16, NULL, (LPOVERLAPPED)psi
					);
